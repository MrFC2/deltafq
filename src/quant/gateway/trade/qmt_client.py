"""
miniQMT 交易（xttrader），类 QmtXtTraderClient。

模块
    market_by_stock_code    证券代码后缀推市场枚举，供按合同号撤单等

对外
    __init__                  userdata_mini 路径、资金账号、会话号（可用环境变量），构造时完成连接
    disconnect                停止并清空引用
...
class QmtXtTraderClient:
    ...
    def __init__(self,
        ...
        if not self.userdata_mini_path:
            raise ValueError("userdata_mini 路径为空，请设置 QMT_USERDATA_MINI 或传入 userdata_mini_path")
        if not os.path.isdir(self.userdata_mini_path):
            raise ValueError(f"userdata_mini 路径不是有效目录: {self.userdata_mini_path}")
        if not self.account_id:
            raise ValueError("account_id 为空，请设置 QMT_ACCOUNT_ID 或传入 account_id")

        xt, account = self._connect()
        self._xt: XtQuantTrader = xt
        self._account: StockAccount = account

    def _connect(self) -> tuple[XtQuantTrader, StockAccount]:
        """创建 XtQuantTrader 实例、连接柜台并订阅资金账号，返回 (xt, account)。"""
        # 创建交易实例并启动
        xt = XtQuantTrader(self.userdata_mini_path, self.session_id)
        xt.start()
        ...
        if rc != 0:
            xt.stop()
            raise RuntimeError(f"XtQuantTrader 连接失败，rc={rc}")
        ...
        if sub != 0:
            xt.stop()
            raise RuntimeError(f"订阅账号失败，sub={sub}（0 为成功）")
